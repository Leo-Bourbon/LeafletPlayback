...
L.Playback.Control = L.Control.extend({
  ...
  _setup: function () {
    ...
    const startTime = playback.getStartTime();
    ...
    $("#speed-slider").slider({
      ...
      slide: function (event, ui) {
        const speed = self._sliderValToSpeed(parseFloat(ui.value));
    ...
    $("#calendar").datepicker({
      ...
      onSelect: function (date) {
        date = new Date(date);
        const time = $("#timepicker").data("timepicker");
        const ts = self._combineDateAndTime(date, time);
    ...
    $("#timepicker")
      ...
      .on("changeTime.timepicker", function (e) {
        ...
        const ts = self._combineDateAndTime(date, e.time);
        playback.setCursor(ts);
        $("#time-slider").slider("value", ts);
      });
    ...
    $("#load-tracks-save").on("click", function (e) {
      const file = $("#load-tracks-file").get(0).files[0];
  ...
  _combineDateAndTime: function (date, time) {
    ...
    let hr = time.hours || time.hour;
    if (time.meridian === "PM" && hr !== 12) hr += 12;
    const min = time.minutes || time.minute;
    const sec = time.seconds || time.second;
  ...
  _loadTracksFromFile: function (file) {
    ...
    reader.onload = function (e) {
      ...
      let tracks = null;
      try {
        tracks = JSON.parse(fileStr);
      } catch (e) {
        /**
         * See if we can do GPX...
         */
        try {
          tracks = L.Playback.Util.ParseGPX(fileStr);
        } catch (e) {
