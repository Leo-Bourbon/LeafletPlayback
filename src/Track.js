...
L.Playback.Track = L.Class.extend({
  initialize: function (geoJSON, options) {
    options = options || {};
    const tickLen = options.tickLen || 250;
    ...
    const sampleTimes = geoJSON.properties.time;

    this._orientIcon = options.orientIcons || false;
    let previousOrientation;

    const samples = geoJSON.geometry.coordinates;
    let currSample = samples[0];
    let nextSample = samples[1];

    let currSampleTime = sampleTimes[0];
    let t = currSampleTime; // t is used to iterate through tick times
    let nextSampleTime = sampleTimes[1];
